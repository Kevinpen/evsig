...
def sigviz(IDR, Format="bar", Group=1, dataset=0):    
    ...
    if Format=="bar": #Draw bar plot
        ...
        figbar.savefig('./app/static/image/sigbar'+str(i)+'.png', dpi=100)
    ...
    if Format == "div": #Draw diversion plot
        ...
        figdiv.savefig('./app/static/image/sigdiv'+str(i)+'g'+str(Group)+'.png', dpi=100)
        plt.clf()
...
def sigpro(IDR, dataset):
    ...
    figpro.savefig('./app/static/image/sigpro'+str(IDR)+'.png', dpi=100)
    plt.clf()
