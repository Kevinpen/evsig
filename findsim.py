...
data = np.genfromtxt("./templates/evolsig_unconstrained.csv", delimiter='\t',skip_header=1)
sig = pd.read_csv("./templates/evolsig_unconstrained.csv",delimiter='\t',low_memory=False)
...
for x in range(np.size(data,0)):
    ...
    for i in range(np.size(data,0)):
        corr[i]=pearsonr(data[x,1:],data[i,1:])[0]
